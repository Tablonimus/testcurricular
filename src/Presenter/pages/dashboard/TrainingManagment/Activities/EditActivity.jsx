...
export default function EditActivity() {
  ...
  useEffect(() => {
    ActivitiesAdapter.getById(id).then((activity) => {
      setCodeValue(activity.code);
      setNameValue(activity.name);
      setTypeValue(activity.type);
      setThematicAreaValue(activity.thematic_area);
      setPresentationValue(activity.presentation);
      setObjectivesValue(activity.objectives);
      setThemesValue(activity.program);
    });
  }, [id]);
  ...
  function handleSubmit() {
    ...
    ActivitiesAdapter.editOneById(id, updatedActivity)
      .then((response) => {
      ...
      .catch((error) => {
        console.log(error)
        alert("Ocurrió un error al actualizar la actividad");
      });
  }
  ...
  return (
    <Card className="h-full w-full py-10">
      <CardHeader variant="gradient" className="p-6 bg-secondary flex">
        <Typography variant="h6" color="white" className="flex items-center gap-3">
          Editar Actividad
        </Typography>
      </CardHeader>
